      ...
      - /public     :LocalServerの公開アクセスポイント、ここに置いたものがアクセス可能。  
          ...
          - /Recognition_rate_LSC   :タッチ方向動作判定ID認証アナライザの認識率評価ページ  
              ...
              - /script ：ID認証アナライザ、IDコードリストのソース、card1-ID1S1T4U3-rm-dbgでデバッグ完成したアナライザソースファイルをこの直下に置く。  
                          ここでアナライザを修正しないこと。  
                - /c : IDコードリスト
                send.php  ：測定完了時に multi-touchcard.comサーバー使用した場合の測定完了メール送信php
                upload.php  ：測定完了時に multi-touchcard.comサーバー使用した場合の測定結果ファイルアップロードphp
              - index.html  :認識率評価用HTML
              - customer-eval.html  :認識率の実施者依存性評価を関係社外のエンドユーザーに依頼するための認識率測定用ページ  
  ...
  - 社外ユーザー評価依頼用認識率評価ページ
        ...
        - 測定は、認証判定タッチ->測定回数カウント(カウントバータッチ)を繰り替えし、10回測定すると自動的に測定Logファイルとタッチ座標データファイルを作成する。
            - ファイルの保存の場所と方法は、以下のパラメータで制御する
                ...
                + mailSendFlg  : 全測定完了時に完了通知Mailを送るかどうかのフラグ Mail送信、Mail送信無し(Finish Mail Send ボタンを非表示にして送信出来ないようにする。) 
